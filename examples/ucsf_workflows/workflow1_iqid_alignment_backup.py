...
if __name__ == "__main__":
    ...
    reupload_paths = data_paths.get("reupload", {})
    if reupload_paths:
        iqid_reupload = reupload_paths.get("iqid_reupload", {})
        if "3d_scans" in iqid_reupload:
            # Construct full path from base_path + relative path
            relative_path = iqid_reupload["3d_scans"]
            data_path = base_path + relative_path if not relative_path.startswith('/') else relative_path
            dataset_used = "reupload/iqid_reupload/3d_scans"
        elif "sequential_scans" in iqid_reupload:
            relative_path = iqid_reupload["sequential_scans"]  
            data_path = base_path + relative_path if not relative_path.startswith('/') else relative_path
            dataset_used = "reupload/iqid_reupload/sequential_scans"
    
    # Check datapush1 as alternative
    if dataset_used == "base_path":
        datapush1_paths = data_paths.get("datapush1", {})
        if datapush1_paths:
            iqid_images = datapush1_paths.get("iqid_images", {})
            if "3d_scans" in iqid_images:
                relative_path = iqid_images["3d_scans"]
                data_path = base_path + relative_path if not relative_path.startswith('/') else relative_path
                dataset_used = "datapush1/iqid_images/3d_scans"
            elif "sequential_10um" in iqid_images:
                relative_path = iqid_images["sequential_10um"]
                data_path = base_path + relative_path if not relative_path.startswith('/') else relative_path
                dataset_used = "datapush1/iqid_images/sequential_10um"
